...
class Case():
    ...
    def createGeometryOld(self, project_path):
        geom = Geometry()
        geom.createGeometryOld(self, project_path)

    def createGeometry(self, project_path):
        geom = Geometry()
        # geom.createGeometry(self,project_path)
        # createGeometry nach der Masterarbeit von Chen (2019)
        geom.createGeometry2(self, project_path)
        # geom.createGeometryNewTopology(self,project_path)

    def createMesh(self, project_path):
        path_createMesh_script = os.path.join(path_script, '..', 'PreProcessing', 'Mesh', 'createMesh.py')

        # subprocess.check_output(['igg111','-batch','-script',path_createMesh_script],stderr=subprocess.STDOUT)
    ...
    #    def createMeshControlLayers(self,project_path):
    #
    #        createMeshControlLayers(self,project_path)
    ...
    def duplicateCascadeMesh(self, project_path):
        ...
        # subprocess.call(['igg111', '-batch','-script',path_createMesh_script],shell=True)
