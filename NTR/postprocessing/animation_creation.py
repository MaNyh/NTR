...
def create(path_to_yaml_dict):
    ...
    assert "resolution_x" in settings["post_settings"][
        "animation_creation"].keys(), "no resolution for animation defined"

    assert "resolution_y" in settings["post_settings"][
        "animation_creation"].keys(), "no resolution for animation defined"

    assert "low_scale_limit" in settings["post_settings"][
        "animation_creation"].keys(), "no low_scale_limit for animation defined"

    assert "high_scale_limit" in settings["post_settings"][
        "animation_creation"].keys(), "no high_scale_limit for animation defined"
