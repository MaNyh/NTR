...
def createProfileData(case):
    ...
    plt.axis('equal')
    output_path = os.path.join(case.casedir,case_dirs["data"])# os.path.dirname(os.path.abspath(path_to_mesh))
    plt.grid()
    ...
    writeOutput(os.path.join(case.casedir,case_dirs["data"]))
...
def calcProfileValues(p_ss, p_ps, x_ss, inte_p_tot1, case, x_ps, y_ss, y_ps, inte_mag_u1, inte_rho1, inte_p1):
    ...
    output_path = os.path.join(case.casedir,case_dirs["data"])
