# -*- coding: utf-8 -*-
"""
Created on Tue Mar 27 16:15:15 2018
...
from NTR.Libs.aeroFunctions import calcCp
from NTR.PostProcessing.Tecplot.Output.writeTecplotFile import writeTecplot1DFile
from NTR.Libs.aeroFunctions import Re_is
from NTR.Libs.aeroFunctions import Ma_is
from NTR.Libs.aeroFunctions import Ma_is_x
from NTR.Libs.aeroFunctions import Ma
from NTR.Libs.aeroFunctions import Re
from NTR.Libs.aeroFunctions import p_t_is
from NTR.Libs.aeroFunctions import T_t_is
from NTR.Libs.thermoFunctions import Sutherland_Law
from NTR.Libs.aeroFunctions import Beta
from NTR.Libs.simFunctions import sort_value3
from NTR.Libs.simFunctions import sort_value2
from NTR.Libs.aeroFunctions import AVDR
from NTR.Libs.aeroFunctions import calcPos2ValuesByAmecke
from NTR.Libs.geomFunctions import getPitchValuesB2BSliceComplete
from NTR.Libs.geomFunctions import GetProfileValuesMidspan


def createProfileData(case, path_midspan_slice, post_slice_1_x, post_slice_2_x):

    [value_names, [values_ss, values_ps]] = GetProfileValuesMidspan(path_midspan_slice)
    ...
    def calcPostSliceValues(x, ind):
        ...
        for ii in xrange(npts):
            pt = points.GetPoint(ii)
            y[ii] = pt[1]
            xx[ii] = pt[0]
            zz[ii] = pt[2]

        if not mapper.GetOutput().GetPointData().GetArray('mag_U'):

            mag_u_array = mapper.GetOutput().GetPointData().GetArray('Mag_U')

        else:

            mag_u_array = mapper.GetOutput().GetPointData().GetArray('mag_U')

        nvls = mag_u_array.GetNumberOfTuples()

        if not mapper.GetOutput().GetPointData().GetArray('U'):
            ux_array = mapper.GetOutput().GetPointData().GetArray('Ux')
            uy_array = mapper.GetOutput().GetPointData().GetArray('Uy')
            uz_array = mapper.GetOutput().GetPointData().GetArray('Uz')
        else:

            ux_array = mapper.GetOutput().GetPointData().GetArray('U')
            uy_array = mapper.GetOutput().GetPointData().GetArray('V')
            uz_array = mapper.GetOutput().GetPointData().GetArray('W')

        rho_array = mapper.GetOutput().GetPointData().GetArray('rho')
        T_array = mapper.GetOutput().GetPointData().GetArray('T')
        p_array = mapper.GetOutput().GetPointData().GetArray('p')
        ...
        for i in xrange(0, nvls):
            mag_u.append(mag_u_array.GetTuple(i)[0])
            ux.append(ux_array.GetTuple(i)[0])
            uy.append(uy_array.GetTuple(i)[0])
            uz.append(uz_array.GetTuple(i)[0])
            rho.append(rho_array.GetTuple(i)[0])
            T.append(T_array.GetTuple(i)[0])
            p.append(p_array.GetTuple(i)[0])
            ma.append(Ma(mag_u[-1], case.kappa, case.R_L, T[-1]))
        ...
        for i in range(len(p)):
            # kappa ma
            p_tot.append(p_t_is(case.kappa, ma[i], p[i]))
            T_tot.append(T_t_is(case.kappa, ma[i], T[i]))

        # mean_mag_u,inte_mag_u=integrate(y,mag_u)
    ...
    zeta = (inte_p_tot1 - inte_p_tot2) / (inte_p_tot1 - case.p_k)

    Ma1 = Ma(inte_mag_u1, case.kappa, case.R_L, inte_T1)
    Ma2 = Ma(inte_mag_u2, case.kappa, case.R_L, inte_T2)

    Ma_is_2 = Ma_is(case.p_k, case.kappa, inte_p1, inte_rho1, inte_mag_u1, case.R_L, inte_T_tot1)
    # Ma_is_2=Ma_is(case.p_k,case.kappa,inte_p1,inte_rho1,inte_mag_u1,case.R_L,inte_T1)
    # Re_is_2=Re_is(case.kappa,case.R_L,case.l,case.As,Ma_is_2,case.p_k,inte_T1,inte_mag_u1,case.c_p,case.Ts)

    Re_is_2 = Re_is(case.kappa, case.R_L, case.l, case.As, Ma_is_2, case.p_k, inte_T_tot1, inte_mag_u1, case.c_p,
                    case.Ts)

    beta1 = 90.0 + math.atan(inte_uy1 / inte_ux1) / 2.0 / math.pi * 360.0
    beta2 = 90.0 + math.atan(inte_uy2 / inte_ux2) / 2.0 / math.pi * 360.0

    nu1 = Sutherland_Law(inte_T1, case.As, case.Ts)
    nu2 = Sutherland_Law(inte_T2, case.As, case.Ts)

    Re1 = Re(inte_rho1, inte_mag_u1, case.l, nu1)
    Re2 = Re(inte_rho2, inte_mag_u2, case.l, nu2)

    AVDR_value = AVDR(inte_rho1, inte_mag_u1, beta1, inte_rho2, inte_mag_u2, beta2)
    delta_beta = beta1 - beta2
    delta_p_static = (inte_p1 - inte_p2) / (inte_p_tot1 - case.p_k)

    y, array_names, values = getPitchValuesB2BSliceComplete(path_midspan_slice, post_slice_2_x)
    ...
    for i in range(len(p_2_y)):
        ...
        pt_2_y.append(p_t_is(case.kappa, Ma(Mag_U_2_y[i], case.kappa, case.R_L, T_2_y[i]), p_2_y[i]))

    Ma2_amecke, beta2_amecke, pt2_amecke, p2_amecke = calcPos2ValuesByAmecke(pt_2_y, beta_2_y, p_2_y, y, inte_p_tot1,
                                                                             kappa=case.kappa)

    zeta_amecke = (inte_p_tot1 - pt2_amecke) / (inte_p_tot1 - case.p_k)
    ma_is_amecke = Ma_is_x(case.kappa, p2_amecke, pt2_amecke)

    def calcProfileValues():
